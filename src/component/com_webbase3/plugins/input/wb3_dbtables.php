<?php
function wb3_dbtables($field, $fieldData)
{
    $query = "
        select 
          TABLE_NAME,        
    ...
    foreach ($rows as $row) {
        unset($selected);
        if ($fieldData[$field->wb3_field] == $row->TABLE_NAME) {
            $selected = "selected";
        }
        $option .= '<option value="' . $row->TABLE_NAME . '" ' . $selected . '>' . $row->TABLE_NAME . '(' . $row->TABLE_SCHEMA . ')</option>';
    }
